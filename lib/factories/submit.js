...
class Submit extends Component {
  ...
  confirmAction(actionComment) {
    const {
      options: {
        form,
        form: { props: { value, options: { onSubmit, requestUploadUrl, url, stackedPlaceHolders } } },
        remote,
      ...
      .then((resp) => {
        ...
        if (fieldsToStore) {
          ...
          fieldsToStore.forEach((field) => {
            ...
            responseStore.response[fieldSplit[1]] = resp[fieldSplit[0]].data.response[fieldSplit[1]] ? resp[fieldSplit[0]].data.response[fieldSplit[1]] : resp[fieldSplit[0]].data.response[0][fieldSplit[1]]
          })
        }
