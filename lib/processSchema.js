...
{
  let response = {}
  const input = stringTemplate(uriTemplate, stackedPlaceholders, /\$\{([0-9a-zA-Z_\.]+)\}/g, '${')
...
async function processSchema(json)
{
  ...
  await processChildrens(json, childrens, filterStrings,
                         key => stringTemplate(json[key], stackedPlaceholders, /\$\{([0-9a-zA-Z_\.]+)\}/g, '${'))
  ...
  if(type === 'string' || type==='number')
  {
    ...
    if('meta' in json){
      await processChildrens(json.meta, json.meta, filterStrings,
                         key => stringTemplate(json.meta[key], stackedPlaceholders, /\$\{([0-9a-zA-Z_\.]+)\}/g, '${'))
    }
