...
async function processSchema(json)
{
  ...
  const placeholdersStack = {...this, ...placeholders}

  // Fill placeholders in string values
  processChildrens(json, childrens, filterStrings,
                   key => stringTemplate(json[key], placeholdersStack))
  ...
  if(type === 'string' && enumInfo)
  {
    ...
    if(uriTemplate)
    {
      const input = stringTemplate(uriTemplate, placeholdersStack)
  ...
  processChildrens(json, childrens, filterObjects,
                   key => processSchema.call(placeholdersStack, json[key]))
