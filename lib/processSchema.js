...
async function loadObjectFields(fields, obj) {
  const apolloFetch = createApolloFetch({ uri: obj.uri })
...
async function processSchema(json) {
  ...
  if (json.preprocessing) {
    await loadObjectFields(json.preprocessing.objectFields, this.object)
  }
  ...
  if (type === 'number' && /^[\d+.\-*\/\(\)]*$/.test(json.value)) {
    try {
      console.warn('evaluating', json.value);
