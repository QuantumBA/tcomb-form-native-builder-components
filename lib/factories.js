...
class Submit extends Component
{
  ...
  _onPress = () =>
  {
    ...
    if(meta.graphql){
      ...
      const mutation = `mutation { ${meta.graphql}(${body_query}) {tag} }`
      
      this.setState({inFlight: true})
      apolloFetch({ query: mutation })
        .then(onSubmit.bind(null, null), error => {console.log(typeof error);onSubmit(error)})
