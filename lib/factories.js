...
class Chips extends Component
{
  ...
  getTemplate()
  {
    ...
    {
      const {chip, chipIcon, container, list, listRow, _palette} = stylesheet

      const {primaryColor} = _palette
      ...
      return <Selectize
        ...
        tintColor={primaryColor} trimOnSubmit={trimOnSubmit}
      ...
      return (
        <View style={[formGroupStyle, {flexDirection: 'row'}]}>
          ...
          <TextField disabled={disabled} editable={false}
            error={hasError && error} labelHeight={0}
