...
class FilesSanitizied extends Files
{
  constructor(props)
  {
    ...
    {
      this.setState({error2: new Error('File name is not encoded as Windows-1252')})
    }
  }
...
FilesSanitizied.transformer =
{
  format(value)
  {
    value = Files.transformer.format(value)

    return Array.prototype.map.call(value, function(file)
    {
      const {lastModified, name, type, uri} = file

      const result = new File([file], windows1252.decode(name, {mode: 'fatal'}),
                              {type, lastModified})
      result.uri = uri
  ...
  parse(files)
  {
    ...
    return Array.prototype.map.call(files, function(file)
    {
      ...
      const result = new File([file], windows1252.encode(name, {mode: 'fatal'}),
                              {type, lastModified})
      result.uri = uri
...
const needsSanitizied = ['android', 'ios'].includes(Platform.OS)
